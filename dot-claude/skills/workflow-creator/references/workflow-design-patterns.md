...
ワークフローのフェーズ数は、実行コスト（Agent呼び出し回数）に直結する。

### フェーズ数の目安

| フェーズ数 | 目安 | 備考 |
|-----------|------|------|
| 3〜5 | 推奨（フラット構造） | ほとんどのユースケースに適切 |
| 6〜7 | 許容（複雑な業務フロー） | サブワークフロー分割を検討する |
| 8以上 | 要分割 | 複合フェーズを `type: workflow` に変換して階層化する |

**2階層上限**: サブワークフロー内にさらにサブワークフローは作成しない（既存ルール）。

### Agent呼び出し数の見積もり

1回の workflow-creator 実行で発生する Agent 呼び出し数の目安:

| フェーズ | 呼び出し数 |
|---------|-----------|
| DESIGN（workflow-designer） | 1〜3回（差し戻しあり） |
| GENERATE（workflow-generator） | 1回 |
| SUB_WORKFLOWS（workflow-generator × N） | サブワークフロー数分 |
| SUBAGENTS（workflow-subagent-generator × N） | 新規エージェント数分 |
| DOMAIN_SKILLS（workflow-domain-skill-generator × N） | 新規スキル数分 |
| REVIEW（workflow-artifact-reviewer） | 1〜3回（Grade C差し戻しあり） |

フェーズ数が多い・エージェント数が多い場合は、DISCOVERYの段階でユーザーに実行コストを予告することを推奨する。
